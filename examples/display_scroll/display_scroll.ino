...
{
    int i;
    mylcd.Set_Addr_Window(mylcd.Get_Display_Width()-40-40, 20, mylcd.Get_Display_Width()-40-1, 59); 
...
void windowScroll(int16_t x, int16_t y, int16_t wid, int16_t ht, int16_t dx, int16_t dy, uint16_t *buf)
{
    ...
    { 
      ...
      {
            mylcd.Read_GRAM(x, y + row, buf,wid, 1);
            mylcd.Set_Addr_Window(x, y + row, x + wid - 1, y + row);
            mylcd.Push_Any_Color(buf + dx, wid - dx, 1,0);
            mylcd.Push_Any_Color(buf + 0, dx, 0,0);
        }
    }
    if (dy) 
    {
      ...
      {
            mylcd.Read_GRAM(x + col, y, buf,1, ht);
            mylcd.Set_Addr_Window(x + col, y, x + col, y + ht - 1);
            mylcd.Push_Any_Color(buf + dy, ht - dy, 1,0);
            mylcd.Push_Any_Color(buf + 0, dy, 0,0);
      }
    }
}
...
void color_test()
{
  ...
  for(i=0;i<cnum;i++)
  {
    for(n=0;n<32;n++)
    {
      mylcd.Set_Draw_color(n*8, n*8, n*8); 
      mylcd.Set_Draw_color(mylcd.Get_Draw_color()&color_mask[i]);
      mylcd.Fill_Rectangle(n*mylcd.Get_Display_Width()/32,(mylcd.Get_Display_Height()/cnum)*i,(n+1)*mylcd.Get_Display_Width()/32,(mylcd.Get_Display_Height()/cnum)*(i+1));
     }
     show_string(color_name[i],0,((mylcd.Get_Display_Height()/cnum)-16)/2+(mylcd.Get_Display_Height()/cnum)*i,2,color_mask[i], BLACK,1);
  }
  delay(1000);
}
...
void text_test(void)
{
    ...
    mylcd.Print_Number_Float(01234.56789, 4, 0, 8, '.', 0, ' '); 
...
void loop() 
{
    ...
    uint16_t scrollbuf[mylcd.Get_Display_Height()]; 
    ...
    for(rotation = 0;rotation<4;rotation++)
    {
        ...
        {
          mylcd.Set_Draw_color(n*8, n*8, n*8); 
          mylcd.Set_Draw_color(mylcd.Get_Draw_color()&color_mask[rotation]);
          mylcd.Fill_Rectangle(n*mylcd.Get_Display_Width()/32,48,(n+1)*mylcd.Get_Display_Width()/32,112);
        }
        ...
        if(mylcd.Get_Rotation()&1)
        {
            max_scroll = mylcd.Get_Display_Width();
         }
         else
         {
            max_scroll = mylcd.Get_Display_Height();
         }
         for(i=1;i<=max_scroll;i++)
         {
            mylcd.Vert_Scroll(0, max_scroll, i);
            delay(10);
          }
          mylcd.Vert_Scroll(0, max_scroll, 0);
          show_string("VERTICAL SCROLL DN",0,168,2,GREEN, BLACK,0); 
         for(i=1;i<=max_scroll;i++)
         {
            mylcd.Vert_Scroll(0, max_scroll, 0-i);
            delay(10);
          }
          delay(1000);
          mylcd.Vert_Scroll(0, max_scroll, 0);
         ...
         for (int16_t i = mylcd.Get_Display_Width(), dx = 4, dy = 0; i > 0; i -= dx)
        {
             windowScroll(0, 216, mylcd.Get_Display_Width(), 16, dx, dy, scrollbuf);
        }
         delay(3000);
     }
     
    mylcd.Invert_Display(true);
    delay(3000);
    mylcd.Invert_Display(false);
    mylcd.Fill_Screen(BLACK);
}
