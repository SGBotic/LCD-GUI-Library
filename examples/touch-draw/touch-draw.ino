// demo of uisng touch funciton to draw on screen
...
void show_color_select_menu(void)
{
   ...
   for(i = 0;i<6;i++)
   {
       mylcd.Set_Draw_color(color_mask[i]);
       mylcd.Fill_Rectangle(i*COLORBOXSIZE, 0, (i+1)*COLORBOXSIZE-1, COLORBOXSIZE/2-1);
   }  
   mylcd.Set_Draw_color(GREEN);
   mylcd.Fill_Round_Rectangle(109, 21, 138,39, 5);
...
void show_pen_size_select_menu(void)
{
  ...
  for(i = 0;i<4;i++)
  {
      mylcd.Print_Number_Int(i+1, (PENBOXSIZE-50)/2+PENBOXSIZE*i, (COLORBOXSIZE/2-16)/2, 0, ' ',10);
  }
   mylcd.Set_Draw_color(RED);
   mylcd.Fill_Rectangle(24, 9, 54, 9);
   mylcd.Fill_Rectangle(84, 8, 114, 10);
   mylcd.Fill_Rectangle(144, 7, 174, 11);
   mylcd.Fill_Rectangle(204, 6, 234, 12);
   mylcd.Set_Draw_color(GREEN);
   mylcd.Fill_Round_Rectangle(109, 21, 138,39, 5);
   show_string("OK",113,23,2,RED, BLACK,1);
}
...
void show_main_menu(void)
{
   ...
   mylcd.Fill_Round_Rectangle(5, 0, 84,39, 5);
   mylcd.Fill_Round_Rectangle(145, 0, 234,39, 5);
   show_string("COLOUR",10,12,2,BLUE, BLACK,1);
   show_string("PENSIZE",149,12,2,BLUE, BLACK,1);
   mylcd.Set_Draw_color(MAGENTA);
   mylcd.Fill_Round_Rectangle(90, 0, 139,39, 5);
...
comme:
  ...
  if (p.z > MINPRESSURE && p.z < MAXPRESSURE) 
  {
      if (p.y < (TS_MINY-5)) 
      {
          ...
          mylcd.Fill_Rectangle(0, COLORBOXSIZE, mylcd.Get_Display_Width()-1, mylcd.Get_Display_Height()-1);
      }
      ...
      p.y = (mylcd.Get_Display_Height()-map(p.y, TS_MINY, TS_MAXY, mylcd.Get_Display_Height(), 0));
         if(p.y < COLORBOXSIZE) 
         {
              if(((p.x>5)&&(p.x < 84))&&!flag_pen) //select color
              {
                ...
                {
                    mylcd.Set_Draw_color(BLACK);
                    mylcd.Fill_Rectangle(0,0,239,39);
                ...
                switch(current_color)
                {
                  case RED:
                  {
                    ...
                    mylcd.Draw_Rectangle(0, 0, COLORBOXSIZE-1, COLORBOXSIZE/2-1);
                    break;  
                  }
                  case YELLOW:
                  {
                       mylcd.Set_Draw_color(WHITE);
                       mylcd.Draw_Rectangle(COLORBOXSIZE, 0, 2*COLORBOXSIZE-1, COLORBOXSIZE/2-1);
                       break; 
                  }
                  case GREEN:
                  {
                       mylcd.Set_Draw_color(WHITE);
                       mylcd.Draw_Rectangle(2*COLORBOXSIZE, 0, 3*COLORBOXSIZE-1, COLORBOXSIZE/2-1);
                       break; 
                   }
                  case CYAN:
                  {
                       mylcd.Set_Draw_color(WHITE);
                       mylcd.Draw_Rectangle(3*COLORBOXSIZE, 0, 4*COLORBOXSIZE-1, COLORBOXSIZE/2-1);
                       break;                 
                  }
                  case BLUE:
                  {
                       mylcd.Set_Draw_color(WHITE);
                       mylcd.Draw_Rectangle(4*COLORBOXSIZE, 0, 5*COLORBOXSIZE-1, COLORBOXSIZE/2-1);
                       break;   
                  }
                  case MAGENTA:  
                  {
                       mylcd.Set_Draw_color(WHITE);
                       mylcd.Draw_Rectangle(5*COLORBOXSIZE, 0, 6*COLORBOXSIZE-1, COLORBOXSIZE/2-1);
                       break;   
                  }
                  default:
                      break;
            ...
            if(((p.x>145)&&(p.x < 234))&&!flag_colour) //select pen size
            {
                ...
                switch(current_pen)
                {
                   ...
                   {
                       mylcd.Set_Draw_color(WHITE);
                       mylcd.Draw_Rectangle(0, 0, PENBOXSIZE-1, COLORBOXSIZE/2-1);
                       break;
                   }
                   case 1:
                   {
                       mylcd.Set_Draw_color(WHITE);
                       mylcd.Draw_Rectangle(PENBOXSIZE, 0, 2*PENBOXSIZE-1, COLORBOXSIZE/2-1);
                       break; 
                   }
                   case 2:
                   {
                       mylcd.Set_Draw_color(WHITE);
                       mylcd.Draw_Rectangle(2*PENBOXSIZE, 0, 3*PENBOXSIZE-1, COLORBOXSIZE/2-1);
                       break;   
                   }
                   case 3:  
                   {
                       mylcd.Set_Draw_color(WHITE);
                       mylcd.Draw_Rectangle(3*PENBOXSIZE, 0, 4*PENBOXSIZE-1, COLORBOXSIZE/2-1);
                       break;                
                   }
                   default:
                       break;
      ...
      if (((p.y-current_pen) > COLORBOXSIZE) && ((p.y+current_pen) < mylcd.Get_Display_Height()))  //drawing
      {
        mylcd.Set_Draw_color(current_color);
       // if(1 == current_pen)
     //   {
      //      mylcd.Draw_Pixel(p.x,  p.y);
     //   }
     //   else 
     //   {
          mylcd.Fill_Circle(p.x,  p.y,current_pen);
      //  }
    }
  }
}
