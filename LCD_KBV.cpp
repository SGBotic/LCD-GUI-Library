...
LCD_KBV::LCD_KBV(uint16_t model,uint8_t cs, uint8_t cd, uint8_t wr, uint8_t rd, uint8_t reset)
{	
...
LCD_KBV::LCD_KBV(int16_t wid,int16_t heg,uint8_t cs, uint8_t cd, uint8_t wr, uint8_t rd, uint8_t reset)
{	
  #ifndef USE_ADAFRUIT_SHIELD_PIN
	  // Convert pin numbers to registers and bitmasks
	  _reset	 = reset;
...
void LCD_KBV::Write_Cmd(uint16_t cmd)
{
	writeCmd16(cmd);
}

void LCD_KBV::Write_Data(uint16_t data)
{
	writeData16(data);
}

void LCD_KBV::Write_Cmd_Data(uint16_t cmd, uint16_t data)
{
	writeCmdData16(cmd,data);
}

//Write a command and N datas
void LCD_KBV::Push_Command(uint16_t cmd, uint8_t *block, int8_t N)
{
  	CS_ACTIVE;
    writeCmd16(cmd);
    while (N-- > 0) 
...
void LCD_KBV::Set_Addr_Window(int16_t x1, int16_t y1, int16_t x2, int16_t y2)
{
	CS_ACTIVE;
...
void LCD_KBV::Push_Any_Color(uint16_t * block, int16_t n, bool first, uint8_t flags)
{
	uint16_t color;
    uint8_t h, l;
...
void LCD_KBV::Push_Any_Color(uint8_t * block, int16_t n, bool first, uint8_t flags)
{
	uint16_t color;
    uint8_t h, l;
	bool isconst = flags & 1;
...
uint16_t LCD_KBV::Color_To_565(uint8_t r, uint8_t g, uint8_t b)
{
	return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | ((b & 0xF8) >> 3);
}

//read value from lcd register 
uint16_t LCD_KBV::Read_Reg(uint16_t reg, int8_t index)
{
	uint16_t ret,high;
    uint8_t low;
//  if (!have_reset)
...
//read graph RAM data
int16_t LCD_KBV::Read_GRAM(int16_t x, int16_t y, uint16_t *block, int16_t w, int16_t h)
{
	uint16_t ret, dummy;
    int16_t n = w * h;
    uint8_t r, g, b, tmp;
...
void LCD_KBV::Draw_Pixe(int16_t x, int16_t y, uint16_t color)
{
...
void LCD_KBV::Fill_Rect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
	int16_t end;
	if (w < 0) 
	{
...
//Scroll display 
void LCD_KBV::Vert_Scroll(int16_t top, int16_t scrollines, int16_t offset)
{
...
//get lcd width
int16_t LCD_KBV::Get_Width(void) const
{
	return width;
}

//get lcd height
int16_t LCD_KBV::Get_Height(void) const
{
	return height;
}

//set clockwise rotation
void LCD_KBV::Set_Rotation(uint8_t r)
{
...
//2  :  180 degree
//3  :  270 degree
uint8_t LCD_KBV::Get_Rotation(void) const
{
	return rotation;
}

//Anti color display 
void LCD_KBV::Invert_Display(boolean i)
{
	CS_ACTIVE;
	uint8_t val = VL^i;
	if(lcd_driver == ID_932X)
...
void LCD_KBV:: init_table8(const void *table, int16_t size)
{
	uint8_t i;
...
void LCD_KBV:: init_table16(const void *table, int16_t size)
{
...
void LCD_KBV::start(uint16_t ID)
{
	reset();
	delay(200);
	switch(ID)
